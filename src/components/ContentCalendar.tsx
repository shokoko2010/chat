...
const ContentCalendar: React.FC<ContentCalendarProps> = ({ posts, onDelete, onPublishNow, publishingPostId }) => {
                                    ...
                                    <div key={post.id} className="group relative">
                                       <div className={`p-2 rounded-md shadow-sm border-l-4 ${
                                        ...
                                        <div className="absolute top-1 left-1 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                                            {!post.isReminder && !hasBeenPublished && (
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); onPublishNow(post.id); }}
                                                    disabled={publishingPostId !== null}
                                                    className="p-1 bg-green-600 text-white rounded-full hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
                                                    title="انشر الآن"
                                                >
