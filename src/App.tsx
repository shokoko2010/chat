...
const App: React.FC = () => {
  ...
  const handleLoadPagesFromBusiness = useCallback(async (businessId: string) => {
    ...
    try {
      ...
      const errorMessage = error.response?.error?.message || error.message || 'خطأ غير معروف';
      alert(`فشل تحميل الصفحات من حافظة الأعمال: ${errorMessage}`);
    } finally {
  ...
  const handleFullHistorySync = useCallback(async (pageTarget: Target) => {
    ...
    try {
      ...
      const errorMessage = error.response?.error?.message || error.message || 'خطأ غير معروف';
      alert(`فشلت مزامنة السجل للهدف ${pageTarget.name}.\nالسبب: ${errorMessage}`);
    } finally {
